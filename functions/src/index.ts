...
* @param {admin.firestore.Timestamp | Date | any} timestamp - UTC timestamp
* @return {Date} Morocco time
*/
function toMoroccoTime(timestamp: admin.firestore.Timestamp | Date | any): Date {
let utcDate: Date;

// Handle different timestamp formats
  if (timestamp instanceof admin.firestore.Timestamp) {
utcDate = timestamp.toDate();
} else if (timestamp instanceof Date) {
utcDate = timestamp;
} else if (timestamp && typeof timestamp.seconds === 'number') {
// Handle Firestore Timestamp-like object
    utcDate = new Date(timestamp.seconds * 1000 + (timestamp.nanoseconds || 0) / 1000000);
...
console.log('Start date type:', typeof startDate, 'Value:', startDate);
...
console.error("Error in scheduleEventTasks:", error);
throw error;
}
}
